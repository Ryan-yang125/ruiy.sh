...
课程官网 assignment, handout, video 都很全。看了下习题，前几章有趣但难度不大，concurrency 和 lisp 做做差不多了，python 懒得看了。
...
C 在编译后的汇编调用函数签名:`CALL memcmp` , 因此只有一个同名函数

C++会根据参数生产不同签名: `CALL memcmp_int_void_p`, 因此重载

About Errors:

Segment Fault: _(NULL) or some address can't be _
